...
def getitem_from_tensor(tensor: "Tensor", key: object) -> object:
    ...
    if isinstance(key, tuple):
        ...
        for k in key:
            if isinstance(k, int):
                result = result.select(dim=dim, index=k)
            elif isinstance(k, slice):
                result = result.slice(dim=dim, start=k.start, end=k.stop, step=k.step or 1)
                dim += 1
            elif isinstance(k, tensor.__class__):
                if result.ndim <= 2:
                    result = result.index_select(dim=dim, index=k.flatten())
                    dim += 1
                else:
                    ...
                    for j in range(indices_flat._shape[0]):
                        picked.append(result.select(dim=dim, index=int(indices_flat.select(0, j).item())))
                    result = cat(picked, dim=dim)
                    dim += 1
            else:
