...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Replicate (clamp) edge values for the padded regions.
    let ih = clamp(i32(oh) - i32(params.pad_top), 0, i32(params.in_h) - 1);
    let iw = clamp(i32(ow) - i32(params.pad_left), 0, i32(params.in_w) - 1);
