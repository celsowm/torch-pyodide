...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Circular padding: wrap the coordinate around with modulo arithmetic.
    var iw = (i32(ow) - i32(params.pad_left)) % i32(params.in_w);
    if (iw < 0) { iw = iw + i32(params.in_w); }

    var ih = (i32(oh) - i32(params.pad_top)) % i32(params.in_h);
    if (ih < 0) { ih = ih + i32(params.in_h); }
