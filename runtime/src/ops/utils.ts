...
function getDM(): DeviceManager {
  if (!_deviceMgr) throw new Error("DeviceManager not set in utils. Call setDeviceManager() first.");
  return _deviceMgr;
}
...
const _shadowMap = new Map<GPUBuffer, number>();

export function createStorageBuffer(_device: GPUDevice, size: number): GPUBuffer {
  const mgr = getDM();
  const { buffer, shadowId } = mgr.createStorageBuffer(size);
  _shadowCounter++;
  _shadowMap.set(buffer, shadowId);
  return buffer;
}

export function getShadowId(buffer: GPUBuffer): number | undefined {
  return _shadowMap.get(buffer);
}
...
export async function readScalar(device: GPUDevice, buffer: GPUBuffer): Promise<number> {
  const mgr = getDM();
  const data = await mgr.readFromGPUBuffer(buffer, 4);
...
export async function readFromGPU(device: GPUDevice, source: GPUBuffer, length: number, dtype: SupportedDType): Promise<number[]> {
  ...
  try {
    const data = await mgr.readFromGPUBuffer(source, byteSize);
    return decodeValuesByDType(data, dtype);
  } catch (err) {
    // Last resort: try reading from shadow copy if device is truly gone
    const shadowId = _shadowMap.get(source);
    if (shadowId !== undefined) {
      const dm = getDM();
      const shadow = (dm as any)._shadowBuffers.get(shadowId);
      if (shadow) {
        console.warn("[readFromGPU] Falling back to shadow copy for buffer data");
        return Array.from(shadow.data as unknown as number[]).slice(0, length);
      }
    }
    throw err;
  }
